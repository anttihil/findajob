...
def main() -> None:
    ...
    exts = (".ts", ".tsx", ".js", ".jsx")
    if not target_file or not any(str(target_file).endswith(ext) for ext in exts):
    ...
    if proposed_content is not None:
        result = subprocess.run(
            ["npx", "eslint", "--stdin", "--stdin-filename", str(target_path)],
    ...
    if result.returncode != 0:
        ...
        reason = (
            f"ESLint issues found in proposed change to {target_path.name}:\n"
            f"{output}\n"
            f"Please fix the lint issues before applying this change."
        )
