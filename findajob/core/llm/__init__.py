...
from findajob.core.llm.factory import get_llm_provider, list_available_providers
from findajob.core.llm.providers.deepseek_api import (
...
class Spend:
    ...
    def record(self, message: Any) -> tuple[dict[str, int], float]:
        ...
        cost = vars(message).get("findajob_cost_usd", 0.0) if message is not None else 0.0
        self.usd += cost
        self.calls += 1
