...
**Exit check:** `careerradar --help` lists no research stage; the scheduler runs search and
score; the job drawer opens without a dossier panel; `careerradar status` prints without a
research section.
...
**Exit check:** `careerradar search seed-cells` reports 588 cells; a dry-run scrape produces
