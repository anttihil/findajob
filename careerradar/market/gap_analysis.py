...
class GapAnalysis:
    def __init__(self, db, config, roles, taxonomy, profile):
        self.db = db
    ...
    # -- corpus loading ---------------------------------------------------------------
    def _load_corpus(self, window_days, location_id=None, role_family=None):
        ...
        for start in range(0, len(ids), 500):
            ...
            for row in self.db.conn.execute(
                f"SELECT job_id, skill, in_title FROM job_skills "
                f"WHERE job_id IN ({placeholders})", chunk
            ):
                skills_by_job.setdefault(row["job_id"], {})[row["skill"]] = bool(
                    row["in_title"]
                )
        return postings, skills_by_job

    def _cell_locations(self):
        return {
            row["id"]: row["location_id"]
            for row in self.db.conn.execute(
                "SELECT id, location_id FROM scrape_cells"
            )
        }

    # -- main entry point --------------------------------------------------------------
    def analyse(self, window_days=90, location_id=None, role_family=None,
                weighting_mode=None):
        postings, skills_by_job = self._load_corpus(
            window_days, location_id, role_family
        )
        mode = weighting_mode or self.analytics_config.get(
        ...
        for posting in postings:
            location = cell_locations.get(posting.get("scrape_cell_id")) or "unknown"
        ...
        if mode == "observed":
            ...
            weights_by_stratum, diagnostics = build_stratum_weights(
                stratum_counts, reference_mix,
                min_stratum_n=self.analytics_config.get("min_stratum_n", 10),
            )

        weight_of = {
            p["id"]: weights_by_stratum.get(stratum_of[p["id"]], 0.0) for p in postings
        }
        ...
        if mode != "observed" and (
            not active or diagnostics["missing_weight"] > max_missing
        ):
            fallback_reason = (
            ...
            diagnostics = dict(
                diagnostics, missing_weight=0.0, weighting_fallback=True
            )
        ...
        return {
            ...
            "provenance": {
                ...
                "window_below_minimum": window_days < self.analytics_config.get(
                    "min_window_days", 30
                ),
                ...
                "cold_start": n_eff_total < self.analytics_config.get(
                    "min_n_eff_for_scope", 20
                ),
            },
        }

    def _per_skill_stats(self, active, good_fit, skills_by_job, weight_of,
                         total_weight, good_fit_weight):
        stats = {}

        def record(skill):
            return stats.setdefault(skill, {
                "weighted_count": 0.0, "raw_count": 0, "weights": [],
                "companies": {}, "in_title": 0, "saturated": 0,
                "blocking_weight": 0.0, "salaries": [],
                "cooccurring": {},
                "familiar_share_sum": 0.0, "familiar_share_n": 0,
            })
            ...
            for skill, in_title in job_skills.items():
                ...
                for other in others:
                    if other in user_skills:
                        entry["cooccurring"][other] = (
                            entry["cooccurring"].get(other, 0) + 1
                        )
        ...
        baseline_salary = weighted_median([
            (p.get("salary_annual_usd"), weight_of[p["id"]])
            for p in active if p.get("salary_annual_usd")
        ])
    ...
    def _score_rows(self, stats, n_eff_total, diagnostics):
        ...
        for skill, entry in stats.items():
            ...
            _, ci_low, ci_high = wilson_interval(
                demand * max(n_eff, 1), max(n_eff, 1)
            )

            company_total = sum(entry["companies"].values()) or 1
            top_company_share = (
                max(entry["companies"].values()) / company_total
                if entry["companies"] else 1.0
            )

            blocking_gap = (
                entry["blocking_weight"] / entry["good_fit_weight"]
                if entry["good_fit_weight"] else 0.0
            )
            adjacency = (
                entry["familiar_share_sum"] / entry["familiar_share_n"]
                if entry["familiar_share_n"] else 0.0
            )
    ...
    def _priority(self, row, gap_weights):
    ...
    def _views(self, rows):
        ...
        return {
            ...
            "suppressed": [
                {"skill": r["skill"], "label": r["label"],
                 "reason": r["suppressed_reason"], "n_raw": r["n_raw"]}
                for r in rows if r["suppressed_reason"]
            ],
        }

    def _empty_result(self, window_days, mode, reason):
        return {
            "window_days": window_days,
            "weighting_mode": mode,
            "rows": [],
            "views": {"validated_strengths": [], "priority_gaps": [],
                      "dead_weight": [], "suppressed": []},
            "provenance": {
                "generated_at": datetime.now(timezone.utc).isoformat(),
                "n_postings": 0, "n_eff": 0.0, "cold_start": True,
    ...
    # -- drill-down --------------------------------------------------------------------
    def skill_detail(self, skill, window_days=90, limit=40):
        ...
        return {
            ...
            "postings": postings,
            "cooccurring": [
                {"skill": r["skill"], "label": self.taxonomy.label(r["skill"]),
                 "n": r["n"], "user_has": self.profile.has(r["skill"])}
                for r in cooccurring
            ],
            "by_role_family": [
                {"role_family": r["role_family"],
                 "label": self.roles.label(r["role_family"]), "n": r["n"]}
                for r in by_family if r["role_family"]
            ],
        }
