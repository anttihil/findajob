...
def covered_days(intervals, window_start, window_end):
    ...
    total = sum(
        (end - start).total_seconds() for start, end in merge_intervals(clipped)
    )
...
def wilson_interval(successes, total, z=1.96):
    ...
    margin = (
        z * math.sqrt(proportion * (1 - proportion) / total
...
def build_stratum_weights(stratum_counts, reference_mix, min_stratum_n=10):
    ...
    """
    usable = {
        key: n for key, n in stratum_counts.items()
    ...
    diagnostics = {
        ...
        "missing_weight": (
            round(1.0 - (target_total / requested_total), 4)
            if requested_total > 0 else 1.0
        ),
    }
...
def hours_old_bucket(hours_old):
...
def comparability_class(location_id, source, hours_old, requested):
    ...
    """
    return (
        f"{location_id}|{source}|{hours_old_bucket(hours_old)}|"
        f"{results_bucket(requested)}"
    )
...
class MarketAnalytics:
    def __init__(self, db, config, roles, taxonomy, profile=None):
        self.db = db
    ...
    def role_supply(self, window_days=30, location_id=None, source="indeed"):
        ...
        for row in observations:
            ...
            record = grouped.setdefault(key, {
                "intervals": [], "observations": 0, "on_topic": 0,
                "saturated": 0, "classes": set(),
            })
            ...
            record["classes"].add(comparability_class(
                row["location_id"], row["source"], row["hours_old"], row["requested"]
            ))

        postings = self._postings_by_family(
            window_start, location_id, source
        )
        ...
        for family, record in grouped.items():
            ...
            counts = postings.get(family, {"n": 0, "companies": set(),
                                           "remote": 0, "salaries": []})
            ...
            rows.append({
                ...
                "saturated_share": round(
                    record["saturated"] / record["observations"], 3
                ) if record["observations"] else None,
        ...
        for row in published:
            row["share"] = (
                round((row["flow_per_day"] or 0) / total_flow, 4)
    ...
    def _postings_by_family(self, window_start, location_id, source):
        ...
        if location_id:
            ...
            query += (
                " AND scrape_cell_id IN "
                "(SELECT id FROM scrape_cells WHERE location_id = ?)"
            )
        ...
        for row in self.db.conn.execute(query, params):
            ...
            record = grouped.setdefault(family, {
                "n": 0, "companies": set(), "remote": 0, "salaries": [],
            })
            record["n"] += 1
    ...
    def _provenance(self, window_days, published, total):
        return {
