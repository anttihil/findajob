...
class MixedScaleTests(unittest.TestCase):
    ...
    def test_a_mixed_corpus_is_refused_rather_than_averaged(self):
...
class RenderTests(unittest.TestCase):
    ...
    def test_the_crosstab_covers_the_grid(self):
        out = render(stats())
        for role in rubric.ROLE_MATCH:
            self.assertIn(role, out)

    def test_the_bands_are_labelled_as_a_projection(self):
    ...
    def test_a_scale_zero_corpus_explains_its_empty_marginals(self):
        out = render(stats(scale_versions={0: 100},
                           marginals={n: {} for n in rubric.DIMENSIONS},
                           crosstab={}, collapsed=[]))
        self.assertIn("Scale 0", out)
        self.assertIn("empty by construction", out)

    def test_a_contradicted_blocker_is_the_flag_that_gets_pointed_at(self):
