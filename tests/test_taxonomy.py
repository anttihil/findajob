...
class TaxonomyIntegrityTests(unittest.TestCase):
    ...
    def test_hash_is_stable_and_content_derived(self):
    ...
    def test_strict_aliases_always_have_context(self):
        """A strict alias without context words would match on capitalization alone."""
        for skill in self.tax.skills.values():
            if skill.strict_aliases:
                self.assertTrue(
                    skill.context, f"{skill.key} has strict_aliases but no context"
                )
...
class LiteralMatchTests(unittest.TestCase):
    ...
    def test_cplusplus_is_found(self):
...
class FalsePositiveGauntletTests(unittest.TestCase):
    ...
    def test_go_the_verb_is_not_the_go_language(self):
        for phrase in [
    ...
    def test_go_the_language_is_found_with_context(self):
        for phrase in [
    ...
    def test_r_the_language_is_found_with_context(self):
        self.assertIn(
            "r_lang",
            self._found("Statistical modelling in R using tidyverse and ggplot."),
        )

    def test_ray_the_noun_is_not_the_ray_framework(self):
        for phrase in [
    ...
    def test_ray_the_framework_is_found_with_context(self):
        self.assertIn(
            "ray",
            self._found("Distributed training with Ray Serve on Anyscale."),
        )

    def test_spark_the_verb_is_not_apache_spark(self):
        for phrase in [
    ...
    def test_spark_the_engine_is_found_with_context(self):
        self.assertIn(
            "spark",
            self._found("Build ETL data pipelines with Apache Spark and PySpark."),
        )

    def test_uv_light_is_not_the_python_packager(self):
        for phrase in [
    ...
    def test_uv_the_packager_is_found_with_context(self):
        self.assertIn(
            "uv_tool",
            self._found("Python dependency management with uv and pyproject.toml."),
        )

    def test_shell_the_noun_is_not_shell_scripting(self):
        self.assertNotIn(
            "shell", self._found("We used to work at Shell before founding this company.")
        )

    def test_shell_scripting_is_found_with_context(self):
        self.assertIn(
            "shell",
            self._found("Comfortable with shell scripting for Linux automation."),
        )

    def test_a_prose_paragraph_with_no_tech_yields_nothing(self):
        prose = (
...
class ExtractionTests(unittest.TestCase):
    ...
    def test_in_title_flag_is_set(self):
    ...
    def test_realistic_posting_extracts_expected_stack(self):
        posting = """
        ...
        for expected in [
            "aws", "ec2", "s3", "iam", "vpc", "terraform", "ansible", "docker",
            "kubernetes", "ci_cd", "github_actions", "python", "golang", "postgresql",
...
class BlockerTests(unittest.TestCase):
    ...
    def test_eu_work_authorization_detected(self):
...
class CanonicalizationTests(unittest.TestCase):
    ...
    def test_unmapped_surfaces_are_reported_for_taxonomy_growth(self):
        _, unmapped = self.tax.canonicalize_verbose(["Python", "Blorpengine 9000"])
        self.assertIn("Blorpengine 9000", unmapped)

    def test_language_human_user_levels_are_explicit(self):
