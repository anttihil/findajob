...
from backend.proxies import (  # noqa: E402
    ENV_VAR,
    apply_proxy_budgets,
    is_rotating,
    load_proxies,
...
class LoadProxiesTests(unittest.TestCase):
    ...
    def test_unset_env_yields_no_proxies(self):
...
class BudgetLiftTests(unittest.TestCase):
    ...
    def test_no_proxies_keeps_the_conservative_budget(self):
    ...
    def test_proxies_lift_linkedin_to_a_description_census(self):
...
class PinningTests(unittest.TestCase):
    def test_pin_returns_exactly_one_endpoint(self):
    ...
    def test_same_cell_and_attempt_is_stable(self):
        self.assertEqual(pin_for(POOL, key=3), pin_for(POOL, key=3))

    def test_different_cells_spread_across_the_pool(self):
        picked = {pin_for(POOL, key=k)[0] for k in range(len(POOL))}
        self.assertEqual(len(picked), len(POOL))

    def test_retry_moves_to_a_different_exit_ip(self):
    ...
    def test_single_endpoint_pool_survives_retries(self):
