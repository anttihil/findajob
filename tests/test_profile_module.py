...
class StructuredOutputTests(unittest.TestCase):
    ...
    def prose(self, text="Here is the profile you asked for."):
        from unittest import mock
    ...
    def parsed(self):
        ...
        return {"parsed": Profile(bio="A engineer."),
                "raw": mock.Mock(response_metadata={"finish_reason": "tool_calls"}),
                "parsing_error": None}

    def test_a_missing_tool_call_is_retried_rather_than_returned_as_none(self):
    ...
    def test_exhausting_the_retries_raises_instead_of_returning_none(self):
    ...
    def test_a_truncated_tool_call_is_reported_as_truncation_not_as_prose(self):
        ...
        raw = mock.Mock(response_metadata={"finish_reason": "length"},
                        invalid_tool_calls=[], content="")
        self.assertIn("output token limit", _no_tool_call_reason(raw))

    def test_synthesize_surfaces_the_error_instead_of_an_attributeerror(self):
        from unittest import mock

        from careerradar.core.llm import StructuredOutputError
        from careerradar.profile.graph import node_synthesize

        model, _ = self.chain([self.prose()] * 3)
        state = {
            "corpus": "docs", "model": "deepseek-v4-pro",
            "claims": {"bio": "b", "skills": [], "apparent_strengths": [],
                       "contradictions": []},
...
class FakeGraph:
    ...
    def __init__(self, parked_at=(), turns=()):
        self.stream_inputs = []
        self.parked_at = parked_at
        self.turns = list(turns)
        self.finished = False

    def stream(self, stream_input, thread, stream_mode=None):
    ...
    def get_state(self, thread):
        if self.finished:
            return FakeSnapshot((), {"approved": True, "turns": self.turns,
                                     "draft": Profile(bio="Done.").model_dump()})
...
class ResumeTests(unittest.TestCase):
    ...
    def build(self, graph, **flags):
        import argparse
        from unittest import mock

        from careerradar.profile.cli import cmd_build

        defaults = {"subcommand": "build", "resume": False, "force": True,
                    "no_interview": False}
        args = argparse.Namespace(**{**defaults, **flags})
        document = mock.Mock(kind="resume", name="cv.md")
        with mock.patch("careerradar.profile.cli.load_config", return_value={}), \
             mock.patch("careerradar.profile.ingest.collect_documents",
             ...
             mock.patch("careerradar.profile.graph.open_checkpointer",
                        return_value=mock.Mock()), \
             mock.patch("careerradar.profile.graph.build_graph", return_value=graph):
            return cmd_build(args)

    def test_resume_continues_the_parked_task_rather_than_restarting(self):
        graph = FakeGraph(parked_at=("synthesize",),
                          turns=[{"topic": "comp", "question": "?", "answer": "90k"}])
    ...
    def test_a_fresh_build_still_passes_the_payload(self):
        graph = FakeGraph()
        self.build(graph)
    ...
    def test_resume_with_nothing_parked_reports_it_instead_of_rebuilding(self):
...
class LevelDiffTests(unittest.TestCase):
    ...
    def diff(self, claim_levels, draft_levels):
        from careerradar.profile.graph import level_changes

        claims = {"skills": [{"key": k, "label": k.title(), "level": v, "evidence": "d"}
                             for k, v in claim_levels.items()]}
        draft = {"skills": [{"key": k, "label": k.title(), "level": v, "evidence": "i"}
                            for k, v in draft_levels.items()]}
        return level_changes(claims, draft)

    def test_a_raised_level_is_reported_with_both_ends(self):
        changes = self.diff({"selenium": 1}, {"selenium": 2})
        self.assertEqual(changes["raised"],
                         [{"key": "selenium", "label": "Selenium", "from": 1, "to": 2}])
        self.assertEqual(changes["lowered"], [])

    def test_a_lowered_level_is_not_quietly_folded_in_with_the_raises(self):
    ...
    def test_a_skill_the_interview_introduced_counts_as_added_not_raised(self):
        changes = self.diff({}, {"selenium": 2})
        self.assertEqual([c["key"] for c in changes["added"]], ["selenium"])
        self.assertEqual(changes["raised"], [])

    def test_synthesis_that_echoes_the_claims_reports_nothing_moved(self):
    ...
    def test_the_no_change_case_says_so_in_words_a_reviewer_can_act_on(self):
    ...
    def test_changes_render_with_level_names_rather_than_numbers(self):
        from careerradar.profile.cli import _render_level_changes
    ...
    def test_missing_claims_do_not_break_the_review_screen(self):
        from careerradar.profile.graph import level_changes

        changes = level_changes(None, {"skills": [
            {"key": "go", "label": "Go", "level": 2, "evidence": "i"}]})
        self.assertEqual(len(changes["added"]), 1)

    def test_the_review_interrupt_carries_the_diff(self):
        from unittest import mock
        ...
        state = {
            "draft": {"skills": [{"key": "selenium", "label": "Selenium", "level": 2,
                                  "evidence": "interview"}]},
            "claims": {"skills": [{"key": "selenium", "label": "Selenium", "level": 1,
                                   "evidence": "doc"}]},
        }
